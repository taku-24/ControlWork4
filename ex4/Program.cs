...
class Program
    {
        ...
        static void SubscribeToCatEvents(Cat cat)
        {
            cat.OnMaxSatiety += c => { Console.WriteLine($"Кот {c.Name} полностью сыт!"); Console.Beep(); };
            cat.OnMaxMood += c => { Console.WriteLine($"Кот {c.Name} полностью счастлив!"); Console.Beep(); };
            cat.OnMaxHealth += c => { Console.WriteLine($"Кот {c.Name} полностью здоров!"); Console.Beep(); };
        }
    }
